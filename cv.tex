% Preamble {{{1

\documentclass{article}

% Packages {{{2
\usepackage{xstring}
\usepackage{hyperref}
\usepackage{geometry}
\usepackage{fontspec}
% }}}2

% Page Layout and style {{{2
\geometry{
	a4paper,       % paper size a4
	vmargin=1in,   % 0.5" margins all round
	hmargin=0.5in, % 0.5" margins all round
}

% Get a good font!!
% \setmainfont{Hoefler Text}

\hypersetup{
    colorlinks=true,   % false: boxed links; true: colored links
    urlcolor=blue      % color of external links
}
% }}}2

% Commands {{{2

% Email (commands) {{{3
\newcommand{\email}[1]{&%
    \IfEqCase{#1}{%
	    {personal}{\href{mailto:\personal}{\personal} \textit{(institution)}\\}
	    {academic}{\href{mailto:\academic}{\academic} \textit{(institution)}\\}
    }[\PackageError{email}{Undefined option to email: #1}{}]%
}
% }}}3

% Section {{{3
\renewcommand{\section}[1]{\\& \multicolumn{2}{l}{\bfseries{\Large{#1}}}\\}
% }}}3

% Entry {{{3
\newcommand{\entry}[3]{\scriptsize{\textsc{#1}} & \multicolumn{2}{l}{#2} \\ #3}
% }}}3

% E(ntry )Head(ing) {{{3
\newcommand{\ehead}[3]{\scriptsize{\textsc{#1}} & \multicolumn{2}{l}{\bfseries{#2}} \\ #3}
% }}}3

% E(ntry i)tem {{{3
\newcommand{\etem}[2]{& #1 & #2 \\}
% }}}3
% }}}2

% Attributes / information {{{2
% Emails {{{3
\newcommand{\academic}{hbh1g18@soton.ac.uk}
\newcommand{\personal}{hugo@heagen.com}
% }}}3

\newcommand{\phone}{&07958997156 \\}

% }}}2

% }}}1

\begin{document}

\noindent\begin{tabular}{l l l}
\section{Contact Details}
\email{academic}
\phone

\section{Education} 
\ehead{2016--2018}{King Edward IV Five Ways}{
              \etem{A-level Politics \& Government}{B}
              \etem{A-level Mathematics}           {B}
              \etem{A-level Economics}             {B}
	      \etem{EPQ}                           {A* (52 / 54)}
}
% A History of Aesthetics in Western Philosophy and an Examination of whether Aesthetics is Properly Considered a Discipline or an Application of Philosophy
\ehead{from 2018}{BA Philosophy, Universty of Southampton}
& \indent\textit{(module marks/100)}\noindent & \\
            \etem{Ancient Greek Philosophy        }{77}
            \etem{Ethics                          }{72}
            \etem{Existentialism and its Origins  }{72}
            \etem{Faith and Reason                }{83}
            \etem{Freedom and Responsibility      }{77}
            \etem{Knowledge \& Mind               }{76}
            \etem{Puzzles about Art \& Literature }{75}
            \etem{Reason and Argument             }{88}
            \etem{Aesthetics                      }{73}
            \etem{Philosophy of Mind              }{76}
            \etem{Kant                            }{75}
            \etem{Appearance and Reality          }{81}
            \etem{Logic                           }{97$^\dag$}
            \etem{Metaphysics                     }{$^\dag$}
            \etem{Metaethics                      }{$^\dag$}
            \etem{Philosophy of Language          }{$^\dag$}
& \dag\ \textit{Provisional} & \\
\section{Awards}
% \entry{2019}{Philosophy Alumni Prize for Best Performance by a 1st year Single Honours Student, Southampton University}{}

\section{Events Attended}
\end{tabular}
\end{document}
