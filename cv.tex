\documentclass{cv}
\usepackage{orcidlink}
\usepackage{hyperref}
\usepackage[utf8]{inputenc}
\usepackage{datatool}
\usepackage{changepage}

\hypersetup{
 pdfauthor={Hugo Heagren},
 pdftitle={Hugo Heagren},
 pdfkeywords={philosophy,academic,academia,university,cv,resume,curriculum vitae},
 pdfsubject={Hugo Heagren, curriculum vitae},
 pdflang={English}}

\usepackage[%
  bibstyle=cv,
  datamodel=cv,
]{biblatex}

\addbibresource{./cv.bib}

% referee data
\DTLloaddb{referees}{./referees.csv}

\author{Hugo Heagren}
\date{\today}
\title{Hugo Heagren}

\begin{document}

\maketitle
\nocite{*}

\section*{Contact Details}
\begin{itemize}
\item \href{mailto:\directlua{tex.sprint{info['email']}}}{\directlua{tex.sprint{info['email']}}}
\item \href{https://hugoheagren.com}{hugoheagren.com}
\item \orcidlinkc{\directlua{tex.sprint{info['orcid']}}}
\end{itemize}

% ** Areas
\begin{areas}
\item[{AOS}]%
  \begin{area}
  \item Philosophy of Language
  \item Philosophy of Mind
  \end{area}
\item[{AOC}]%
  \begin{area}
  \item Epistemology
  \item Metaphysics
  \item Philosophy of Psychology
\end{area}
\end{areas}

\section*{Education}
\printbibliography[check=Education]{}

\section*{Employment}
\printbibliography[check=Post]{}

\section*{Publications \secnote{\invited{} invited, otherwise peer reviewed}}
\printbibliography[check=Publication]{}

\section*{Visits}
\printbibliography[check=Visit]{}

\section*{Major Grants}
\printbibliography[check=MajorFunding/Award]{}

\section*{Minor Grants, Awards \& Scholarships}
\printbibliography[check=MinorFunding/Award]{}

\section*{Talks \secnote{\invited{} invited, \departmental{} departmental, otherwise refereed}}
\printbibliography[check=Presentation]{}

\section*{Comments}
\printbibliography[check=Comment]{}

\section*{Teaching}
\subsection*{As Convener}
\printbibliography[check=Teaching+Convener]{}

\subsection*{As Guest Lecturer}
\printbibliography[check=Teaching+GuestLecturer]{}

\subsection*{As Teaching Assistant}
\printbibliography[check=Teaching+TA]{}

\section*{Academic Service}
\begin{description}
% 2025-11-19
\item[{2025}] Job market preparation workshop for postgraduate students at King's College London
\item[{2025}] Co-organiser for 12th International Philosophy of
Medicine Round table \& Sowerby Anniversary Lecture
\item[{2025}] Lead organiser for Workshop on new work in theory of reference, King's College London
\item[{2024--2025}] Technical manager for the \href{https://www.philosophyandmedicine.org/}{Sowerby Philosophy and
Medicine Project}
\item[{2023--2024}] Organizer for \href{https://www.lemm-london.co.uk/}{Language, Epistemology, Metaphysics and Mind}
\item[{2023-2025}] Administrator for the \href{https://philosophyarts.co.uk/}{Centre for Philosophy and Art}
\item[{2023-2025}] Technical manager for the \href{https://www.ai-humanity-london.com/}{London AI and Humanity Project}
% Pretty sure this what December of 2023
\item[{2023}] Undergraduate workshop on philosophical writing (co-run with other graduate students)
\item[{2022}] Outreach teaching for Gonville \& Caius college---lectures on
history of philosophy of language and my own research
\end{description}

\section*{Submission Review}

\begin{adjustwidth}{\dategutter}{}
  \directlua{r = info['reviews']; table.sort(r); tex.print(table.concat(r,', '))}
\end{adjustwidth}

\section*{Referees \secnote{Most recent first}}

\newcounter{reftblcol}
\begin{tabular}{@{} p{8cm} p{8cm} @{}}
  \DTLforeach{referees}{%
    \name=name,\title=title,\position=position,\note=note,\affiliation=affiliation,\email=email%
  }{%
  \begin{itemize}[nosep]
    \item \textbf{\title\ \name{\ifx\position\empty\else\ (\position)\fi}}
    \item \note
    \item \affiliation
    \item \href{mailto:\email}{\email}
  \end{itemize}
  \stepcounter{reftblcol}
  % if counter < 2 then align, else reset counter and end
  \ifnum\value{reftblcol}<2
    % This is a tokenizer hack. See
    % https://tex.stackexchange.com/a/131284/238079
    \uppercase{&}
  \else
    \setcounter{reftblcol}{0} \\
  \fi
  }
\end{tabular}

\end{document}

% * File local variables

% Local Variables:
% compile-command: "make cv"
% End:
